...
<?
	...
	if ($_REQUEST["track"] != "") {
		...
		$art = mf(mq("select `artist` from `[p]musicplayer` where id='{$artist}' limit 1"));
		...
		'."
		<div style='float: left; padding-right: 10px;'><span style='color: #555 !important;'>Artist:</span> {$music_artist} <span style='color: #555 !important;'>// Track:</span> {$track_name}</div> <div class='vote'>1</div><div class='vote nay'>0</div><div class='clear'></div>";
	}
