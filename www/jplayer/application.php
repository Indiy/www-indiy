...
$getArtistImage = mq("select * from mydna_musicplayer where id='{$user_id}'");
$cnt = 0;



if($row["oauth_provider"]=='facebook')
	$xml .= '<VIDLIBDATA avatar_img="https://graph.facebook.com/'.$row["username"].'/picture" playlist_id="0" vidnum_selected="2">';
elseif($row["oauth_provider"]=='twitter')
	$xml .= '<VIDLIBDATA avatar_img="'.$row['profile_image_url'].'" playlist_id="0" vidnum_selected="2">';
