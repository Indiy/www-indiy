...
if ($browser == true || $_GET["debug"] == "true"){
	...
	while ($video = mf($loadvideo)) {
		...
		if( !($cv % 3) && ($cv != 0) ) {  ++$row_counter; $artist_videos .= '</div><div class="video-row video-row-' . $row_counter . '">'; }
		$artist_videos .= '
			<li id="video_'.$cv.'" class="playlist_video_master';
				
				if( !(($cv+1) % 3) ) $artist_videos .= ' last';
				
				$artist_videos .= '">
				<div class="playlist_video"><img src="artists/images/'.$video_image.'" border="0" /></div>
			</li>'."\n";
		++$cv;
	}	
	$artist_videos .= '</div>';
...
$(document).ready(function(){
	...
	if( '<?= $_SERVER['REMOTE_ADDR']; ?>' == '75.73.10.205' ) $("#jquery_jplayer").jPlayer("pause");
...
-->
			...
			<div id="logo">
				...
				<? if ($artist_logo) { ?><img src="<?=trueSiteUrl();?>/artists/images/<?=$artist_logo;?>" /><? } ?>
				</div>
